...
class XGtkmm3Template : public Gtk::Window {
    ...
    protected:
        ...
        void toggle_template() {
            ...
            for (const auto &var_name : vars_names) {
                ...
                grid_vars->attach(*label_var_name, 0, row_idx, 1, 1);
                label_var_name->set_halign(Gtk::ALIGN_START);
                label_var_name->set_text(var_name);
                Gtk::Entry *entry_var = Gtk::make_managed<Gtk::Entry>();
                grid_vars->attach(*entry_var, 1, row_idx, 1, 1);
                entry_var->set_text("");
            ...
            button_result->signal_clicked().connect(
                [this, buffer, entries_vars, true_curr_row]() {
                    std::string new_text = static_cast<Glib::ustring>(
                        true_curr_row[template_cols.body]);
                    int var_num = 1;
                    for (const auto &entry_var : entries_vars) {
                        ...
                        while ((pos = new_text.find(placeholder, pos)) !=
                               std::string::npos) {
                            new_text.replace(pos, placeholder.length(),
                                             replacement);
                    ...
                    if (pipe) {
                        fwrite(new_text.c_str(), 1, new_text.size(), pipe);
                        pclose(pipe);
                    }
                });
    ...
    private:
        ...
        void setup_data() {
            ...
            // Example of hardcoded template
            make_hardcoded_template(liststore_templates, template_cols,
                                    "hardcoded stdout", "std::string &out",
                                    "stdout, hardcoded, reference",
                                    "std::cout << $1 << std::endl;");
